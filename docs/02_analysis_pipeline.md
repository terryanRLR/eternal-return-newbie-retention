...
두 HTML 대시보드에는 전처리 코드 없이 **완성된 집계 JSON이 그대로 박혀 있습니다** (`const D = {...}`, `const RAW_DATA = {...}`, `const PATCH_DATA = {...}`, `const EXTRA_DATA = {...}`). 즉 이 JSON을 만든 Python/SQL 집계 스크립트 자체가 레포에 없습니다.
